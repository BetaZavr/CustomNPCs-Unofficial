...
public class GuiNpcSpawner extends GuiNPCInterface2
   ...
   public void buttonEvent(GuiButtonNop button) {
      switch (button.id) {
         ...
         case 14: {
            if (!isDead) { return; }
            Packets.sendServer(new SPacketNpcJobSpawnerRemove(-1, true));
            break;
         } // clear Dead
   ...
   public void subGuiClosed(Screen gui) {
      ...
      if (selector.showingClones == 2) {
         ...
         if (selected.isEmpty()) { Packets.sendServer(new SPacketNpcJobSpawnerAdd(isDead, selected, selector.activeTab, new CompoundTag())); }
      }
      else {
         CompoundTag nbtNpc = selector.getCompound();
         if (nbtNpc != null && selector.spawnData instanceof JobSpawnerNbtData jobData) {
            jobData.load(nbtNpc);
            Packets.sendServer(new SPacketNpcJobSpawnerAdd(isDead, "", 0, jobData.save()));
         }
      }
   ...
   public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
      if (!hasSubGui()) {
         ...
         graphics.fill(guiLeft + 355, guiTop + 13, guiLeft + 416, guiTop + 99, 0xFF808080);
         graphics.fill(guiLeft + 356, guiTop + 14, guiLeft + 415, guiTop + 98, 0xFF000000);
         matrixStack.popPose();
      }
   ...
   public void setGuiData(CompoundTag compound) {
      if (compound.contains("NPCData", 10)) {
         ...
         if (entityO.isPresent() && entityO.get() instanceof LivingEntity entity) { select = entity; }
         return;
      } // Entity set
      // job data
         ...
         for (int i = 0; i < job.size(type); i++) {
            ...
            if (sd != null) {
               MutableComponent key = Component.empty()
                       ...
                       .append(sd.getTitle());
               if (sd instanceof JobSpawnerNbtData jobData) {
                  key.append(Component.literal(jobData.isClientClone() ? " (Client)" : " (Mob)")
                          .withStyle((jobData.isClientClone() ? ChatFormatting.GREEN : ChatFormatting.RED)));
               }
               else { key.append(Component.literal(" (Server)").withStyle(ChatFormatting.AQUA)); }
               list.add(key);
            }
         }
   ...
   private void resetEntity() {
      ...
      if ((isDead ? deadScroll : aliveScroll).getNormalSelected().getContents() instanceof TranslatableContents tr) { sel = tr.getKey(); }
      if (sel.isEmpty()) {
         select = null;
         return;
      }
      ...
      select = null;
      if (sd != null) {
         if (sd instanceof JobSpawnerNbtData jobData) { select = jobData.getEntity().getMCEntity(); } // client / mob
