...
public class GuiNpcManageTransporters extends GuiContainerNPCInterface2<ContainerNPCTransports>
    ...
    public GuiNpcManageTransporters(ContainerNPCTransports containerIn, Inventory inv, Component titleIn) {
    ...
    public void init() {
        ...
        if (categories.hasSelected()) {
            ...
            if (locations.hasSelected()) {
                ...
                addLabel(10, guiLeft + imageWidth - font.width(idt) - 4, guiTop + 8, idt)
                        .setSize(30, 10);
                for (int i = 0; i < 3; i++) {
    ...
    protected void renderBg(@Nonnull GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
        ...
        if (locations != null && locations.hasSelected()) {
            for (int slotId = 0; slotId < 10; ++slotId) {
                graphics.blit(GuiBasic.RESOURCE_SLOT, guiLeft + container.getSlot(slotId).x - 1, guiTop + container.getSlot(slotId).y - 1,
                        0, 0, 18, 18);
            }
        }
    }
    ...
    public void render(@Nonnull GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (categories != null && categories.hasSelected()) {
            graphics.hLine(guiLeft + 212, guiLeft + imageWidth - 3, guiTop + 178, 0x80000000);
            graphics.vLine(guiLeft + 211, guiTop + 4, guiTop + imageHeight + 12, 0x80000000);
            if (locations.hasSelected()) {
                graphics.vLine(guiLeft + 271, guiTop + 4, guiTop + 111, 0x80000000);
                graphics.hLine(guiLeft + 212, guiLeft + 270, guiTop + 110, 0x80000000);
            }
            graphics.vLine(guiLeft + 418, guiTop + 4, guiTop + imageHeight + 12, 0x80000000);
        }
    }
    @Override
    public void save() {
            ...
            if (category.id == container.location.category.id) {
                for (TransportLocation loc : new ArrayList<>(category.locations.values())) {
                    Component locKey = Component.empty()
                            .append(Component.literal("ID: " + loc.id + " \"").withStyle(ChatFormatting.GRAY))
    ...
    public void subGuiClosed(Screen subgui) {
        if (subgui instanceof SubGuiEditText gui && !gui.text[0].isEmpty()) {
            CompoundTag compound = new CompoundTag();
            compound.putInt("CategoryId", -1);
            compound.putString("CategoryTitle", gui.text[0]);
            ...
            case 2: {
                if (!textField.getValue().isEmpty()) {
                    ...
                    locations.replace(sel, Component.empty()
                            .append(Component.literal("ID: " + container.location.id + " \"").withStyle(ChatFormatting.GRAY))
                            .append(Component.translatable(container.location.name).withStyle(ChatFormatting.RESET))
                            .append(Component.literal("\"").withStyle(ChatFormatting.GRAY)));
                }
                break;
            } // loc name
