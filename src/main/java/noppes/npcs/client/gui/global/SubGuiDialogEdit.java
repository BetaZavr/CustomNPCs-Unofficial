...
public class SubGuiDialogEdit
   ...
   public SubGuiDialogEdit(EntityNPCInterface npcIn, Dialog dialogIn, Screen gui) {
      super(npcIn);
   ...
   public void buttonEvent(GuiButtonNop button) {
      switch (button.id) {
         ...
         case 8: dialog.quest = -1; init(); break;
         ...
         case 14: dialog.mail = new PlayerMail(); init(); break;
   ...
   public void unFocused(GuiTextFieldNop textField) {
      switch (textField.id) {
         ...
         case 2: dialog.sound = textField.getResourceLocation(null); break;
   ...
   public void subGuiClosed(Screen subgui) {
      if (subgui instanceof GuiTextAreaScreen gui) { dialog.text = gui.text; }
      else if (subgui instanceof SubGuiNpcDialogOption) { setSubGui(new SubGuiNpcDialogOptions(npc, dialog, this)); }
      else if (subgui instanceof SubGuiNpcCommand gui) { dialog.command = gui.command; }
      else if (subgui instanceof SubGuiQuestSelection gui) {
         if (gui.getQuest() != null) {
            dialog.quest = gui.getQuest().id;
            init();
         }
      }
      else if (subgui instanceof SubGuiSoundSelection gui) {
         if (gui.resource != null) {
            getTextField(2).setValue(gui.resource.toString());
            unFocused(getTextField(2));
         }
      }
      else if (subgui instanceof SubGuiTextureSelection gui) {
         if (gui.resource == null) { return; }
         dialog.texture = gui.resource.toString();
         init();
      }
   }
   ...
   public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
      ...
      if (!hasSubGui()) {
         graphics.vLine(guiLeft + 196, guiTop + 24, guiTop + 159, 0xFF808080);
         graphics.hLine(guiLeft + 4, guiLeft + imageWidth - 5, guiTop + 159, 0xFF808080);
      }
   }

   @Override
   public void setGuiData(CompoundTag compound) {
      if (compound != null && compound.contains("MinimumID", 3) && dialog.id != compound.getInt("MinimumID")) {
         Packets.sendServer(new SPacketDialogRemove(dialog.id));
         dialog.id = compound.getInt("MinimumID");
         Packets.sendServer(new SPacketDialogSave(dialog.category.id, dialog.save(new CompoundTag())));
         init();
      }
   }
