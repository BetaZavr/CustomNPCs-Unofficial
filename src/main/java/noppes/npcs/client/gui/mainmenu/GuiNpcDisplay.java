...
public class GuiNpcDisplay
        extends GuiNPCInterface2 implements ITextfieldListener, IGuiData {
   ...
   public GuiNpcDisplay(EntityNPCInterface npc) {
      ...
      display = npc.display;
      baseSize = npc.baseSize;
      if (npc instanceof EntityCustomNpc && this.display.getModel() != null) {
         ModelData modeldata = ((EntityCustomNpc) npc).modelData;
         baseSize = new EntityDimensions(modeldata.entity.getBbWidth(), modeldata.entity.getBbHeight(), false);
      }
   ...
   public void init() {
      ...
      int tw = font.width(Component.translatable("display.size"));
      int x3 = x2 - 2 - tw;
      ...
      tw = font.width(Component.translatable("display.tint"));
      x3 = x2 - 2 - tw;
      ...
      tw = font.width(Component.translatable("display.hitbox"));
      x3 = x2 - 2 - tw;
      ...
      addLabel(lID++, x2 + 72, y + 5, "W:")
              .setSize(12, 10);
      float hitBoxWidth = npc.getBbWidth();
      float hitBoxHeight = npc.getBbHeight();
      if (npc instanceof EntityCustomNpc mNpc) {
         float scaleHead = 1.0f, scaleBody = 1.0f;
         if (display.getModel() != null) {
            ModelData modeldata = mNpc.modelData;
            ...
            hitBoxWidth = modeldata.entity.getBbWidth();
            hitBoxHeight = modeldata.entity.getBbHeight();
         }
      ...
      addTextField(12, x2 + 81, y, 30, 20, df.format(displaySize[0]))
              .setMinMaxDefault(-1.0d, 7.5d, displaySize[0])
              .setHoverTexts("display.hover.hitbox.width", ("" + Math.round(baseSize.width * 1000.0) / 1000.0).replace(".", ","), ("" + Math.round(hitBoxWidth * 1000.0) / 1000.0).replace(".", ","));
      addLabel(lID++, x2 + 115, y + 5, "H:")
              .setSize(12, 10);
      addTextField(13, x2 + 125, y, 30, 20, df.format(displaySize[1]))
              .setMinMaxDefault(-1.0d, 15.0d, displaySize[1])
              .setHoverTexts("display.hover.hitbox.height", ("" + Math.round(baseSize.height * 1000.0) / 1000.0).replace(".", ","), ("" + Math.round(hitBoxHeight * 1000.0) / 1000.0).replace(".", ","));
      // Next normal
      y += 23;
      ...
      addButton(10, x1, y, false, display.getBossbar(),
              ...
              .setSize(110, 20);
      tw = font.width(Component.translatable("gui.color"));
      x3 = x2 - 2 - tw;
   ...
   public void buttonEvent(GuiButtonNop button) {
      switch (button.id) {
         ...
         case 1: NoppesUtil.openGUI(player, new GuiCreationEntities(npc)); break;
         case 2: {
            display.setSkinUrl("");
   ...
   @Override
   public void subGuiClosed(Screen subgui) { init(); }
   ...
   public void setGuiData(CompoundTag compound) {
      if (compound.contains("MarkovGeneratorId", 3)) {
