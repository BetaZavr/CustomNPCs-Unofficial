...
public class GuiNpcAdvanced extends GuiNPCInterface2 implements IGuiData {
   ...
   public void init() {
      ...
      MutableComponent mess = Component.translatable("advanced.menu.hover.role." + npc.role.getType());
   ...
   public void setGuiData(CompoundTag compound) {
      if (compound.contains("RoleData")) {
         ...
         switch (npc.role.getEnumType()) {
            case TRADER -> setSubGui(new SubGuiNpcSelectTrader(((RoleTrader) npc.role).getMarketID()));
            case FOLLOWER -> NoppesUtil.requestOpenGUI(EnumGuiType.SetupFollower);
            case BANK -> setScreen(new GuiNpcBankSetup(npc));
            case TRANSPORTER -> setScreen(new GuiNpcTransporter(npc));
            case COMPANION -> setScreen(new GuiNpcCompanion(npc));
            case DIALOG -> NoppesUtil.openGUI(player, new GuiRoleDialog(npc));
         }
      }
      else if (compound.contains("JobData")) {
         npc.job.load(compound);
         switch (npc.job.getEnumType()) {
            case BARD -> NoppesUtil.openGUI(player, new GuiNpcBard(npc));
            case HEALER -> NoppesUtil.openGUI(player, new GuiNpcHealer(npc));
            case GUARD -> NoppesUtil.openGUI(player, new GuiNpcGuard(npc));
            case ITEM_GIVER -> NoppesUtil.requestOpenGUI(EnumGuiType.SetupItemGiver);
            case FOLLOWER -> NoppesUtil.openGUI(player, new GuiNpcFollowerJob(npc));
            case SPAWNER -> NoppesUtil.openGUI(player, new GuiNpcSpawner(npc));
            case CONVERSATION -> NoppesUtil.openGUI(player, new GuiNpcConversation(npc));
            case PUPPET -> NoppesUtil.openGUI(player, new GuiNpcPuppet(this, (EntityCustomNpc) npc));
            case FARMER -> NoppesUtil.openGUI(player, new GuiJobFarmer(npc));
         }
      }
      else if (compound.contains("NpcInteractLines", 10)) { npc.advanced.load(compound); }
      else if (compound.contains("NpcInv", 9)) { npc.inventory.load(compound); }
      else if (compound.contains("MovementType", 3)) { ais.load(compound); }
      init();
   }
   ...
   public void save() {
      if (hasChanges) {
         ...
         hasChanges = false;
         // New from Unofficial (BetaZavr)
         Packets.sendServer(new SPacketNpcRoleSave(npc.role.save(new CompoundTag())));
         Packets.sendServer(new SPacketNpcJobSave(npc.job.save(new CompoundTag())));
   ...
   public void subGuiClosed(Screen subgui) {
      if (subgui instanceof SubGuiNpcSelectTrader gui) {
         hasChanges = true;
         ((RoleTrader) npc.role).setMarket(gui.id);
         save();
      }
   }
