...
public class CustomAnimationHandler {
    ...
    // Animation settings
    protected final Map<AnimationKind, List<Integer>> data = new HashMap<>();
    protected final Map<Integer, Long> waitData = new HashMap<>(); // animation ID, time
    ...
    protected boolean completeAnimation = false;
    protected ResourceLocation animationSound = null;
    protected boolean canSetBaseRotationAngles = true;
    protected int timeTicks = -1;
    protected int speedTicks = -1;
    ...
    public CustomAnimationHandler(LivingEntity main) {
        entity = main;
        checkData();
    }
    ...
    public void stopAnimation() {
        if (activeAnimation != null) {
            if (activeAnimation.type != AnimationKind.EDITING_All && activeAnimation.type != AnimationKind.EDITING_PART) {
                int ticks = (int) (entity.level().getGameTime() - startAnimationTime);
        ...
        if (animationSound != null && entity.level().isClientSide()) { MusicController.Instance.stopSound(animationSound, SoundSource.AMBIENT); }
        animationSound = null;
    ...
    public void calculationAnimationData(float partialTicks) {
        ...
        int ticks = Math.max(0, (int) (entity.level().getGameTime() - startAnimationTime));
        ...
        if (animationSound == null &&
            ...
            MusicController.Instance.playSound(SoundSource.AMBIENT, currentFrame.sound.toString(), entity.getX(), entity.getY(), entity.getZ(), 1.0f, 1.0f);
    ...
    private float calcValue(float value_0, float value_1, float speed, float ticks, boolean isSmooth, float partialTicks) {
        ...
        if (isSmooth) { // Apply antialiasing if necessary
            progress = -0.5f * (float) Math.cos(progress * Math.PI) + 0.5f;
        }
    ...
    public void load(CompoundTag compound) {
        if (!compound.contains("AllAnimations", 9) && !compound.contains("MovementAnimations", 9)) { return; }
        stopAnimation();
        data.clear();
        movementAnimation.clear();
        AnimationController aData = AnimationController.getInstance();
        for (int c = 0; c < compound.getList("AllAnimations", 10).size(); c++) {
            CompoundTag nbtCategory = compound.getList("AllAnimations", 10).getCompound(c);
            int t = nbtCategory.getInt("Category");
            if (t < 0) { t *= -1; }
            ...
            int tagType = Objects.requireNonNull(nbtCategory.get("Animations")).getId();
            if (tagType == 11) { // OLD version
                ...
                int listType = ((ListTag) Objects.requireNonNull(nbtCategory.get("Animations"))).getElementType();
                if (listType == 10 && entity != null && !entity.level().isClientSide()) { // OLD in main CNPCs mod
                    for (int i = 0; i < nbtCategory.getList("Animations", 10).size(); i++) {
                        CompoundTag nbt = nbtCategory.getList("Animations", 10).getCompound(i);
                        int id = nbt.getInt("ID");
                        String name = entity.getName() + "_" + nbt.getString("Name");
                        AnimationConfig anim = aData.getAnimation(id);
                        if (entity.level().getEntity(entity.getId()) != null && anim == null || !anim.getName().equals(name)) {
                            boolean found = false;
                            if (anim != null) {
                    ...
                    for (int i = 0; i < nbtCategory.getList("Animations", 3).size(); i++) {
                        int id = nbtCategory.getList("Animations", 3).getInt(i);
                        if (!list.contains(id)) { list.add(id); }
                    }
                }
            }
        ...
        if (compound.contains("MovementAnimations", 9) && entity != null && entity.level().isClientSide()) {
            for (int c = 0; c < compound.getList("MovementAnimations", 10).size(); c++) {
                CompoundTag nbt = compound.getList("MovementAnimations", 10).getCompound(c);
                movementAnimation.put(AnimationKind.get(nbt.getInt("Type")), nbt.getInt("ID"));
            }
    ...
    public void save(CompoundTag compound) {
        checkData();
        ListTag allAnimations = new ListTag();
        for (AnimationKind type : data.keySet()) {
            if (data.get(type) == null || data.get(type).isEmpty()) { continue; }
            CompoundTag nbtCategory = new CompoundTag();
            nbtCategory.putInt("Category", type.get());
            ListTag animations = new ListTag();
            for (int id : data.get(type)) { animations.add(IntTag.valueOf(id)); }
            nbtCategory.put("Animations", animations);
            allAnimations.add(nbtCategory);
        }
        compound.put("AllAnimations", allAnimations);
        if (entity != null && !entity.level().isClientSide() && !movementAnimation.isEmpty()) {
            ListTag movementAnimations = new ListTag();
            for (AnimationKind ak : new ArrayList<>(movementAnimation.keySet())) {
                CompoundTag nbt = new CompoundTag();
                nbt.putInt("Type", ak.ordinal());
                nbt.putInt("ID", movementAnimation.get(ak));
                movementAnimations.add(nbt);
            }
            compound.put("MovementAnimations", movementAnimations);
        }
    }
    ...
    public boolean isAnimated() {
        ...
        boolean isEdit = activeAnimation.type == AnimationKind.EDITING_All || activeAnimation.type == AnimationKind.EDITING_PART;
        return !entity.level().isClientSide() ? !completeAnimation : stage != EnumAnimationStages.Waiting || isEdit || activeAnimation.type.isMovement();
    }
    ...
    public void updateTime() {
        ...
        if (!AnimationController.getInstance().hasAnimation(activeAnimation.id) && stage != EnumAnimationStages.Ending && stage != EnumAnimationStages.Waiting) {
            ...
            startAnimationTime = entity.level().getGameTime() + 1;
            return;
        }
        int ticks = Math.max(0, (int) (entity.level().getGameTime() - startAnimationTime));
        int speed;
        ...
        if (!isEdit && activeAnimation.type.isMovement() && stage != EnumAnimationStages.Ending && stage != EnumAnimationStages.Waiting) {
            ...
            if (isRav) {
                ...
                if (entity instanceof Player) {
                    if (entity.getLastHurtMobTimestamp() - entity.tickCount > 300) {
                        stop = true;
                    }
                } else {
        ...
        if (stage == EnumAnimationStages.Looping) {
            ...
            if (ticks >= speed) {
                ...
                if (frameId == 0) {
                    ...
                    startAnimationTime = entity.level().getGameTime() + activeAnimation.endingFrameTicks.get(frameId - 1) + 1;
                }
    ...
    private void startEvent(AnimationEvent event) {
        ...
        if (event instanceof AnimationEvent.UpdateEvent && !entity.level().isClientSide()) { return; }
    ...
    private void runAnimation(AnimationConfig anim, AnimationKind type) {
        ...
        if (!entity.level().isClientSide() && isEdit) { return; }
        ...
        startAnimationTime = entity.level().getGameTime();
        completeAnimation = false;
        calculationAnimationData(0.0f);
    }

    public Map<Integer, Integer> resetWalkAndStandAnimations() {
        if (entity.level().isClientSide() || (activeAnimation != null && !completeAnimation)) { return null; }
    ...
    public void loadBaseAnimations(Map<Integer, Integer> map) {
        AnimationController aData = AnimationController.getInstance();
        movementAnimation.clear();
        for (int key : map.keySet()) {
            AnimationConfig anim = aData.getAnimation(map.get(key));
            if (anim != null) { movementAnimation.put(AnimationKind.get(key), anim.id); }
        }
    }
    ...
    private AnimationKind getCurrentMovementAnimation() {
        ...
        if (!movementAnimation.isEmpty() && (activeAnimation == null || isMoving)) {
            ...
            if (movementAnimation.containsKey(AnimationKind.AIM) && !isMoving) {
                if (entity instanceof Player) {
                    if (entity.getUseItem().getItem() instanceof BowItem) { base = AnimationKind.AIM; }
                } else {
            ...
            if (base == null) {
                ...
                // WATER
                if (base == null) {
                    AABB bBox = entity.getBoundingBox().inflate(-0.1D, -0.4D, -0.1D);
                    if (entity.isInWater() || entity.isInLava() || entity.level().containsAnyLiquid(bBox)) {
    ...
    public boolean hasAnimation(AnimationKind type, int id) {
        return data.get(type) != null && data.get(type).contains(id);
    }

    public boolean canSetBaseRotationAngles() { return canSetBaseRotationAngles; }

    public boolean canBeAnimated() {
        return stage != EnumAnimationStages.Waiting || (!movementAnimation.isEmpty() && (activeAnimation == null || activeAnimation.type.isMovement()) && entity.level().getGameTime() % 3 == 0);
    }
    ...
    public ItemStack getCurrentHeldStack(boolean isMainHand) {
        ...
        switch(isMainHand ? currentFrame.getHoldRightStackType() : currentFrame.getHoldLeftStackType()) {
            case 0: return isMainHand ? entity.getMainHandItem() : entity.getOffhandItem(); // normal
            case 1: {
                if (entity instanceof Player) { return new ItemStack(Items.ARROW); }
                else {
                    iStack = ((EntityNPCInterface) entity).inventory.getProjectile();
                    break;
                }
            } // projectile
            case 2: return isMainHand ? entity.getMainHandItem() : entity.getOffhandItem(); // revers hand
            case 3: {
                iStack = isMainHand ? currentFrame.getHoldRightStack() : currentFrame.getHoldLeftStack();
                break;
            } // custom
            case 4: return entity.getItemBySlot(EquipmentSlot.HEAD);
            case 5: return entity.getItemBySlot(EquipmentSlot.CHEST);
            case 6: return entity.getItemBySlot(EquipmentSlot.LEGS);
            case 7: return entity.getItemBySlot(EquipmentSlot.FEET);
        }
        return iStack == null ? isMainHand ? entity.getMainHandItem() : entity.getOffhandItem() : iStack.getMCItemStack();
    }
